...
#'for Fisher's exact test applied to 2 x 2 contingency tables
...
fisher.pvalues.support<-function(counts, alternative = "greater", input = "noassoc"){
  ...
  input <- char.expand(input, c("noassoc", "marginal", "HG2011"))
  switch (input, noassoc={
    A11 <- counts[, 1]
    A12 <- counts[, 2]
    n <- A11 + A12
    #A21<- counts[,3]
  ...
  switch(alternative, greater=
           for (i in 1:number.items){
             ...
             pCDFlist[[i]] <- phyper(x-1, A1.[i], A2.[i], n[i], lower.tail = FALSE)
         ...
         less=
           for (i in 1:number.items){
             x <- 0:k[i]
             pCDFlist[[i]] <- phyper(x, A1.[i], A2.[i], n[i], lower.tail = TRUE)
             # the "+1" below is because vectors start with index 1 in R: x[A11[i]+1]=A11[i]
             raw.pvalues[i] <- pCDFlist[[i]][A11[i]+1]
           },
         two.sided=
